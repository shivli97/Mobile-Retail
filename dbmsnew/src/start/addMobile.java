...
public class addMobile implements ActionListener {
    ...
    private JLabel lblMemory = new JLabel("Memory(GB)");
    private JLabel lblRAM = new JLabel("RAM(GB)");
    private JLabel lblCameraFront = new JLabel("Camera (Front)");
    private JLabel lblCameraBack = new JLabel("Camera (Back)");
    private JLabel lblBattery = new JLabel("Battery(mAh)");
    private JLabel lblBluetooth = new JLabel("Bluetooth(yes/no)");
    private JLabel lblCostPrice = new JLabel("Cost Price");
    ...
    private JTextField txtCostPrice = new JTextField();
    ...
    public addMobile() {
        ...
        lblCostPrice.setBounds(50, 350, 100, 20);
        backImage.add(lblCostPrice);
        ...
        txtCostPrice.setColumns(20);
        txtCostPrice.setBounds(160, 350, 150, 18);
        backImage.add(txtCostPrice);
    ...
    public void actionPerformed(ActionEvent e) {
        try {
            ...
            int CameraFront, CameraBack, Memory, RAM, CostPrice;
            if(txtMemory.getText().length()==0) 
            	Memory = 0;
            else
            ...
            if(txtCameraFront.getText().length()==0) 
            	CameraFront = 0;
            else
            	CameraFront = Integer.parseInt(txtCameraFront.getText());
            
            if(txtCameraBack.getText().length()==0)
            	CameraBack = 0;
            else
            	CameraBack = Integer.parseInt(txtCameraBack.getText());
            
            if(txtCostPrice.getText().length()==0)
            	CostPrice = 0;
            else
            ...
            if (e.getSource() == btnReset) 
            {
                ...
                stmt.executeUpdate("INSERT INTO mobile VALUES('" + MobileID + "','" + Name + "','" + Company + "','" + Type + "','" + LaunchDate + "','" + DispSize + "', '" + Resolution + "','" + OS + "','" + CPU + "','" + Memory + "','" + RAM + "','" + CameraFront + "','" + CameraBack + "','" + Battery + "','" + Bluetooth + "','"+ CostPrice +"')");

                JOptionPane.showMessageDialog(null, "Successfully inserted into the database", "Success!!!", JOptionPane.INFORMATION_MESSAGE);
                resetAll();

            } else if (e.getSource() == btnModify) {
                String str = "UPDATE mobile SET NAME=?, COMPANY=?, TYPE=?,LAUNCH_DATE=?, DISP_SIZE=?, RESOLUTION=?, OS=?, CPU=?, MEMORY=?, RAM=?, CAMERA_FRONT=?, CAMERA_BACK=?, BATTERY=?, BLUETOOTH=?, COST_PRICE=? WHERE MOBILE_ID=?";
                ...
                psmt.setString(9, Integer.toString(Memory));
                psmt.setString(10, Integer.toString(RAM));
                psmt.setString(11, Integer.toString(CameraFront));
                psmt.setString(12, Integer.toString(CameraBack));
                psmt.setString(13, Battery);
                psmt.setString(14, Bluetooth);
                psmt.setString(15, Integer.toString(CostPrice));
                psmt.setString(16, MobileID);
            ...
            {            	
                ...
                if (rs.next()) 
                {
                    ...
                    String Memory2 = rs.getString("MEMORY");
                    String RAM2 = rs.getString("RAM");
                    String CameraFront2 = rs.getString("CAMERA_FRONT");
                    String CameraBack2 = rs.getString("CAMERA_BACK");
                    ...
                    txtOS.setText(OS);
                    txtCPU.setText(CPU);
                    txtMemory.setText(Memory2);
                    txtRAM.setText(RAM2);
                    txtCameraFront.setText(CameraFront2);
                    txtCameraBack.setText(CameraBack2);
